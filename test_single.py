...
WEIGHTS_PATH = HERE / "pinn_trained_weights.pth"
...
def physics_baseline(
    ...
    env = IsotopeEnvironment(phi=flux_phi, sigma_ra226=1e-24, neutron_energy_ev=energy_ev)
...
def pinn_predict(
    ...
    x = torch.tensor(
        [[t_norm, phi_nn, e_nn, init226, init225, init_ac]],
        dtype=torch.float32,
    )
    with torch.no_grad():
        pred = model(x)
...
def emit_verdict(
    *,
    ...
    budget_slack = max(atol, 1e-6 * max(total_0, 1.0))
    ...
    if title.startswith("B ") and pinn_ok and not math.isnan(n_ac):
        if ode_ac > initial_ac225 + atol:
            ...
            print()
            if rel < 0.25:
                print("TRIO B: PASS -- Ac-225 vs ODE within ~25% (full-tank transmutation track).")
            else:
                print(
    ...
    if pinn_ok and model is not None:
        try:
            ...
            if "size mismatch" in str(e):
                print("PINN: checkpoint incompatible with current 6-input IsotopePINN.")
...
def main() -> None:
    ...
    if args.legacy:
        run_scenario(model, pinn_ok=pinn_ok, **scenario_c)
        return
    ...
    run_scenario(
        model,
        title="B - Full tank: Ra-226 = 1e22 + reactor flux",
        time_h=250.0,
        flux_phi=1.0e14,
        energy_ev=0.025,
        n226_0=1.0e22,
